...
class tmhomebottomcms extends Module
{
	function __construct()
	{
		$this->name = 'tmhomebottomcms';
		$this->tab = 'Other';
		$this->version = '1.0';
		...
		$this->displayName = $this->l('Templatemela - Home Bottom CMS');
	...
	{
		if (!parent::install()
			|| !$this->registerHook('TemplateMelaFooterDiv')
	...
	function hookTemplateMelaFooterDiv($params){return $this->hookdisplayRightColumn($params);}
	...
	public function hookdisplayRightColumn($params)
	{
		...
		if (file_exists(dirname(__FILE__).'/content.xml'))
		{
			...
			if ($xml = simplexml_load_file(dirname(__FILE__).'/content.xml'))
			{
				...
				return $this->display(__FILE__, 'tmhomebottomcms.tpl');
			}
		}
		return false;
	}

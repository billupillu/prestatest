...
require (dirname(__FILE__).'/megamenu.class.php');

class Blockmegamenu extends Module
{
	private $_menu = '';
	private $_html = '';
	private $user_groups;
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('Top Mega menu');
	...
	public function install()
	{
		if (!parent::install() ||
			...
			!Configuration::updateGlobalValue('MOD_BLOCKMEGAMENU_ITEMS', 'CAT1,CMS1,CMS2,PRD1') ||
			!Configuration::updateGlobalValue('MOD_BLOCKMEGAMENU_SEARCH', '0') ||
	...
	public function installDb()
	{
		...
		CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'linksmegamenu` (
			...
			Db::getInstance()->execute('
			 CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'linksmegamenu_lang` (
	...
	{
		if (!parent::uninstall() ||
			!Configuration::deleteByName('MOD_BLOCKMEGAMENU_ITEMS') ||
			!Configuration::deleteByName('MOD_BLOCKMEGAMENU_SEARCH') ||
			!$this->uninstallDB())
			return false;
		return true;
	}
	...
	{
		Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'linksmegamenu`');
		Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'linksmegamenu_lang`');
		return true;
	}
	...
	public function getContent()
	{
		...
		{
			if (Configuration::updateValue('MOD_BLOCKMEGAMENU_ITEMS', Tools::getValue('items')))
				$this->_html .= $this->displayConfirmation($this->l('Settings Updated'));
			else
				$this->_html .= $this->displayError($this->l('Unable to update settings'));
			Configuration::updateValue('MOD_BLOCKMEGAMENU_SEARCH', (bool)Tools::getValue('search'));
			$update_cache = true;
		}
		else if (Tools::isSubmit('submitBlocktopmenuLinks'))
		{
			...
			{
				MegaMenuLinks::add(Tools::getValue('link'), Tools::getValue('label'), Tools::getValue('new_window', 0), (int)Shop::getContextShopID());
		...
		else if (Tools::isSubmit('submitBlocktopmenuRemove'))
		{
			...
			MegaMenuLinks::remove($id_linksmenutop, (int)Shop::getContextShopID());
			Configuration::updateValue('MOD_BLOCKMEGAMENU_ITEMS', str_replace(array('LNK'.$id_linksmenutop.',', 'LNK'.$id_linksmenutop), '', Configuration::get('MOD_BLOCKMEGAMENU_ITEMS')));
		...
		else if (Tools::isSubmit('submitBlocktopmenuEdit'))
		{
			...
			{
				$tmp = MegaMenuLinks::getLinkLang($id_linksmenutop, $id_shop);
			...
			{
				MegaMenuLinks::update(Tools::getValue('link'), Tools::getValue('label'), Tools::getValue('new_window', 0), (int)$id_shop, (int)$id_linksmenutop, (int)$id_linksmenutop);
		...
		<fieldset>
			...
			<form action="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'" method="post" id="form">
				...
				<div class="margin-form">
					<input type="text" name="items" id="itemsInput" value="'.Configuration::get('MOD_BLOCKMEGAMENU_ITEMS').'" size="70" />
				</div>
				</div>
		...
		$links = MegaMenuLinks::gets($id_lang, null, (int)Shop::getContextShopID());
		foreach ($links as $link)
		{
			if ($link['label'] == '')
			{
				$link = MegaMenuLinks::get($link['id_linksmenutop'], $default_language, (int)Shop::getContextShopID());
		...
		$this->_html .= '</select><br/>
				...
				<div class="margin-form">
					<input type="checkbox" name="search" id="s" value="1"'.((Configuration::get('MOD_BLOCKMEGAMENU_SEARCH')) ? ' checked=""': '').'/>
				</div>
				<p class="center">
...
<div class="margin-form">';
		...
		$links = MegaMenuLinks::gets((int)$id_lang, null, (int)Shop::getContextShopID());
	...
	{
		return explode(',', Configuration::get('MOD_BLOCKMEGAMENU_ITEMS'));
	}
	...
	private function makeMenuOption()
	{
		...
		foreach ($menu_item as $item)
		{
			...
			switch (substr($item, 0, strlen($values[1])))
			{
				...
				case 'LNK':
					$link = MegaMenuLinks::get((int)$id, (int)$id_lang, (int)$id_shop);
					if (count($link))
					{
						if (!isset($link[0]['label']) || ($link[0]['label'] == ''))
						{
							$default_language = Configuration::get('PS_LANG_DEFAULT');
							$link = MegaMenuLinks::get($link[0]['id_linksmenutop'], (int)$default_language, (int)Shop::getContextShopID());
						}
			...
			switch (substr($item, 0, strlen($value[1])))
			{
				...
				case 'LNK':
					...
					if (count($link))
					{
						...
						{
							$default_language = Configuration::get('PS_LANG_DEFAULT');
							$link = MegaMenuLinks::get($link[0]['id_linksmenutop'], $default_language, (int)Shop::getContextShopID());
						}
	...
	private function getCategory($id_category, $id_lang = false, $id_shop = false)
	{
		...
		$pth = "http://".$_SERVER["SERVER_NAME"].implode('/', $path_array);
		...
		if (!empty($is_intersected))
		{
			...
			{				
				...
				foreach ($children as $child) {
					...
					$i++;
					$this->_menu .= '<div id="c_'.(int)$child['id_category'].'" class="categoryImg test">
					<img src="'.$pth.'/img/c/'.(int)$child['id_category'].'-category_default.jpg"/></div>';
				...
				$this->_menu .= '</ul><div id="c_'.$id_category.'" class="categoryImg">
				<img src="'.$pth.'/img/c/'.$id_category.'-category_default.jpg"/></div></div>';
	...
	public function hookDisplayTop($param)
	{
		...
		$smarty_cache_id = 'blockmegamenu-'.$this->page_name.'-'.(int)$this->context->shop->id.'-'.implode(', ',$this->user_groups).'-'.(int)$this->context->language->id.'-'.(int)Tools::getValue('id_category').'-'.(int)Tools::getValue('id_manufacturer').'-'.(int)Tools::getValue('id_supplier').'-'.(int)Tools::getValue('id_cms').'-'.(int)Tools::getValue('id_product');
		Tools::enableCache();
		if (!$this->isCached('blockmegamenu.tpl', $smarty_cache_id))
		{
			$this->makeMenu();
			$this->smarty->assign('MENU_SEARCH', Configuration::get('MOD_BLOCKMEGAMENU_SEARCH'));
		...
		$html = $this->display(__FILE__, 'blockmegamenu.tpl', $smarty_cache_id);
	...
	{
		$this->_clearCache('blockmegamenu.tpl');
	}
}

...
class MegaMenuLinks
{
	public static function gets($id_lang, $id_linksmenutop = null, $id_shop)
	{
		...
		$sql = 'SELECT l.id_linksmenutop, l.new_window, ll.link, ll.label
				FROM '._DB_PREFIX_.'linksmegamenu l
				LEFT JOIN '._DB_PREFIX_.'linksmegamenu_lang ll ON (l.id_linksmenutop = ll.id_linksmenutop AND ll.id_lang = '.(int)$id_lang.' AND ll.id_shop='.(int)$id_shop.')
	...
	public static function getLinkLang($id_linksmenutop, $id_shop)
	{
		$ret = Db::getInstance()->executeS('
			...
			FROM '._DB_PREFIX_.'linksmegamenu l
			LEFT JOIN '._DB_PREFIX_.'linksmegamenu_lang ll ON (l.id_linksmenutop = ll.id_linksmenutop AND ll.id_shop='.(int)$id_shop.')
			WHERE 1
	...
	{
		Db::getInstance()->delete('linksmegamenu', 'id_linksmenutop = '.(int)$id_linksmenutop.' AND id_shop = '.(int)$id_shop);
		Db::getInstance()->delete('linksmegamenu_lang', 'id_linksmenutop = '.(int)$id_linksmenutop);
	}

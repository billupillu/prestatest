...
class tmhomerightcms extends Module
{
	function __construct()
	{
		$this->name = 'tmhomerightcms';
		$this->tab = 'Other';
		$this->version = '1.0';
		...
		$this->displayName = $this->l('Templatemela - Home right CMS');
	...
	public function hookdisplayRightColumn($params)
	{
		...
		if (file_exists(dirname(__FILE__).'/content.xml'))
		{
			...
			if ($xml = simplexml_load_file(dirname(__FILE__).'/content.xml'))
			{
				...
				return $this->display(__FILE__, 'tmhomerightcms.tpl');
			}
		}
		return false;
	}

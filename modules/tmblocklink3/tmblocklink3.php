...
class tmblocklink3 extends Module
{
	/* @var boolean error */
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('Templatemela - Footer Support block');
	...
	public function install()
	{
		if (!parent::install() ||
			...
			CREATE TABLE '._DB_PREFIX_.'tmblocklink3 (
			`id_tmblocklink3` int(2) NOT NULL AUTO_INCREMENT, 
			`url` varchar(255) NOT NULL,
			`new_window` TINYINT(1) NOT NULL,
			PRIMARY KEY(`id_tmblocklink3`))
			ENGINE='._MYSQL_ENGINE_.' default CHARSET=utf8') ||
			!Db::getInstance()->execute('
			CREATE TABLE '._DB_PREFIX_.'tmblocklink3_shop (
			`id_tmblocklink3` int(2) NOT NULL AUTO_INCREMENT, 
			`id_shop` int(2) NOT NULL,
			PRIMARY KEY(`id_tmblocklink3`, `id_shop`))
			ENGINE='._MYSQL_ENGINE_.' default CHARSET=utf8') ||
			!Db::getInstance()->execute('
			CREATE TABLE '._DB_PREFIX_.'tmblocklink3_lang (
			`id_tmblocklink3` int(2) NOT NULL,
			`id_lang` int(2) NOT NULL,
			`text` varchar(64) NOT NULL,
			PRIMARY KEY(`id_tmblocklink3`, `id_lang`))
			ENGINE='._MYSQL_ENGINE_.' default CHARSET=utf8') ||
			!Configuration::updateValue('PS_tmblocklink3_TITLE', array('1' => 'Block link3', '2' => 'Bloc lien')))
			return false;
		return true;
	}
	...
	{
		if (!parent::uninstall() ||
			!Db::getInstance()->execute('DROP TABLE '._DB_PREFIX_.'tmblocklink3') ||
			!Db::getInstance()->execute('DROP TABLE '._DB_PREFIX_.'tmblocklink3_lang') ||
			!Db::getInstance()->execute('DROP TABLE '._DB_PREFIX_.'tmblocklink3_shop') ||
			!Configuration::deleteByName('PS_tmblocklink3_TITLE') ||
			!Configuration::deleteByName('PS_tmblocklink3_URL'))
			return false;
		return true;
	}
	...
	public function hookLeftColumn($params)
	{
		...
		$this->smarty->assign(array(
			'tmblocklink3_links' => $links,
			'title' => Configuration::get('PS_tmblocklink3_TITLE', $this->context->language->id),
			'url' => Configuration::get('PS_tmblocklink3_URL'),
			'lang' => 'text_'.$this->context->language->id
		));
		if (!$links)
			return false;
		return $this->display(__FILE__, 'tmblocklink3.tpl');
	}
	...
	public function hookFooter($params)
	{
		...
		if (!$links)
			return false;
		return $this->display(__FILE__, 'tmblocklink3_footer.tpl');
	}
	...
	public function getLinks()
	{
		...
		$sql = 'SELECT b.`id_tmblocklink3`, b.`url`, b.`new_window`
				FROM `'._DB_PREFIX_.'tmblocklink3` b';
		if (Shop::isFeatureActive() && Shop::getContext() != Shop::CONTEXT_ALL)
			$sql .= ' JOIN `'._DB_PREFIX_.'tmblocklink3_shop` bs ON b.`id_tmblocklink3` = bs.`id_tmblocklink3` AND bs.`id_shop` IN ('.implode(', ', Shop::getContextListShopID()).') ';
		$sql .= (int)Configuration::get('PS_tmblocklink3_ORDERWAY') == 1 ? ' ORDER BY `id_tmblocklink3` DESC' : '';
		...
		{
			$result[$i]['id'] = $link['id_tmblocklink3'];
			$result[$i]['url'] = $link['url'];
			$result[$i]['newWindow'] = $link['new_window'];
			// Get multilingual text
			if (!$texts = Db::getInstance()->executeS('SELECT `id_lang`, `text` 
																	FROM '._DB_PREFIX_.'tmblocklink3_lang 
																	WHERE `id_tmblocklink3`='.(int)$link['id_tmblocklink3']))
				return false;
	...
	public function addLink()
	{
		...
		{
			if (!Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'tmblocklink3 SET `url` = \''.pSQL($_POST['url']).'\', `new_window` = '.(isset($_POST['newWindow']) ? 1 : 0).' WHERE `id_tmblocklink3` = '.(int)$id_link))
				return false;
			if (!Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'tmblocklink3_lang WHERE `id_tmblocklink3` = '.(int)$id_link))
				return false;
				
			foreach ($languages as $language)
				if (!empty($_POST['text_'.$language['id_lang']]))
		 	 	{
					if (!Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'tmblocklink3_lang VALUES ('.(int)$id_link.', '.(int)($language['id_lang']).', \''.pSQL($_POST['text_'.$language['id_lang']]).'\')'))
						return false;
		 	 	}
				else
					if (!Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'tmblocklink3_lang VALUES ('.(int)$id_link.', '.$language['id_lang'].', \''.pSQL($_POST['text_'.$id_lang_default]).'\')'))
						return false;
		}
		else
		{
			if (!Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'tmblocklink3 
			...
			foreach ($languages as $language)
				if (!empty($_POST['text_'.$language['id_lang']]))
				{
					if (!Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'tmblocklink3_lang 
																VALUES ('.(int)$id_link.', '.(int)$language['id_lang'].', \''.pSQL($_POST['text_'.$language['id_lang']]).'\')'))
						return false;
				}
				else
					if (!Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'tmblocklink3_lang VALUES ('.(int)$id_link.', '.(int)($language['id_lang']).', \''.pSQL($_POST['text_'.$id_lang_default]).'\')'))
						return false;
		}

		Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'tmblocklink3_shop WHERE id_tmblocklink3='.(int)$id_link);

		if (!Shop::isFeatureActive())
		{
			Db::getInstance()->insert('tmblocklink3_shop', array(
				'id_tmblocklink3' => (int)$id_link,
				'id_shop' => (int)Context::getContext()->shop->id,
			));
		}
		else
		{
			$assos_shop = Tools::getValue('checkBoxShopAsso_tmblocklink3');
			if (empty($assos_shop))
				return false;
	...
	{
		return (Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'tmblocklink3 WHERE `id_tmblocklink3` = '.(int)$_GET['id']) &&
					Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'tmblocklink3_shop WHERE `id_tmblocklink3` = '.(int)$_GET['id']) &&
					Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'tmblocklink3_lang WHERE `id_tmblocklink3` = '.(int)$_GET['id']));
	}
	...
	public function updateTitle()
	{
		...
		if (!Configuration::updateValue('PS_tmblocklink3_TITLE', $result))
			return false;
		return Configuration::updateValue('PS_tmblocklink3_URL', $_POST['title_url']);
	}
	...
	public function getContent()
	{
		...
		{
			if (Configuration::updateValue('PS_tmblocklink3_ORDERWAY', (int)(Tools::getValue('orderWay'))))
	...
	private function _displayForm()
	{
		...
		/* Title */
		$title_url = Configuration::get('PS_tmblocklink3_URL');
		if (!Tools::isSubmit('submitLinkAdd'))
		{
			if ($id_link = (int)Tools::getValue('id_link'))
			{
				...
				SELECT *
				FROM '._DB_PREFIX_.'tmblocklink3 b
				LEFT JOIN '._DB_PREFIX_.'tmblocklink3_lang bl ON (b.id_tmblocklink3 = bl.id_tmblocklink3)
				WHERE b.id_tmblocklink3='.(int)$id_link);
				if ($res)
					foreach ($res as $row)
					{
		...
		<fieldset>
			...
			$this->_html .= '
				...
				if (Shop::isFeatureActive() && count($shops) > 1)
				{
					...
					$helper->table = 'tmblocklink3';
					$helper->identifier = 'id_tmblocklink3';
		...
		foreach ($languages as $language)
			$this->_html .= '
					<div id="title_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $id_lang_default ? 'block' : 'none').'; float: left;">
						<input type="text" name="title_'.$language['id_lang'].'" value="'.Tools::safeOutput(($this->error && isset($_POST['title'])) ? $_POST['title'] : Configuration::get('PS_tmblocklink3_TITLE', $language['id_lang'])).'" /><sup> *</sup>
					</div>';

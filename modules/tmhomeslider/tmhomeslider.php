...
include_once(_PS_MODULE_DIR_.'tmhomeslider/tmHomeSlide.php');



class TmHomeSlider extends Module

{
...

	{
		...
		$this->displayName = $this->l('Templatemela - HomePage Slider');

		$this->description = $this->l('Adds an image slider for your homepage slider.');
...

	{
		...
		if (parent::install() && $this->registerHook('TemplateMelaHomeSlider') && $this->registerHook('actionShopDataDuplication'))
...

	{
...

		{
...

			{
				...
				$slide->title[$language['id_lang']] = 'MainBanner '.$i;

				$slide->url[$language['id_lang']] = '#';

				$slide->image[$language['id_lang']] = 'main-banner-'.$i.'.jpg';
...

	{
		...
		$res = (bool)Db::getInstance()->execute('
			...
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'tmhomeslider` (
		...
		$res &= Db::getInstance()->execute('
			...
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'tmhomeslider_slides` (
		...
		$res &= Db::getInstance()->execute('
			...
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'tmhomeslider_slides_lang` (
...

	{
...

		{
			...
			$to_del = new TmHomeSlide($slide['id_slide']);
		...
		return Db::getInstance()->execute('
			...
			DROP TABLE IF EXISTS `'._DB_PREFIX_.'tmhomeslider`, `'._DB_PREFIX_.'tmhomeslider_slides`, `'._DB_PREFIX_.'tmhomeslider_slides_lang`;
...

	{
		...
		if (Tools::isSubmit('id_slide') && $this->slideExists((int)Tools::getValue('id_slide')))
			...
			$slide = new TmHomeSlide((int)Tools::getValue('id_slide'));
		...
		$this->_html .= '<p class="clear">'.$this->l('Recommended width: 980px.').'</p>';
...

	{
...

		{
			...
			$slide = new TmHomeSlide((int)Tools::getValue('delete_id_slide'));
...

	{
		...
		$this->smarty->assign('tmhomeslider_slides', $slides);
	...
	public function hookTemplateMelaHomeSlider()

	{
		...
		return $this->display(__FILE__, 'tmhomeslider.tpl');
...

	{
		...
		INSERT IGNORE INTO '._DB_PREFIX_.'tmhomeslider (id_homeslider_slides, id_shop)

		SELECT id_homeslider_slides, '.(int)$params['new_id_shop'].'

		FROM '._DB_PREFIX_.'tmhomeslider
...

	{
		...
		$row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
				...
				FROM `'._DB_PREFIX_.'tmhomeslider_slides` hss, `'._DB_PREFIX_.'tmhomeslider` hs
...

	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			FROM '._DB_PREFIX_.'tmhomeslider hs

			LEFT JOIN '._DB_PREFIX_.'tmhomeslider_slides hss ON (hs.id_homeslider_slides = hss.id_homeslider_slides)

			LEFT JOIN '._DB_PREFIX_.'tmhomeslider_slides_lang hssl ON (hss.id_homeslider_slides = hssl.id_homeslider_slides)

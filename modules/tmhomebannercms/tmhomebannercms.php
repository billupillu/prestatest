...
class tmhomebannercms extends Module
{
	function __construct()
	{
		$this->name = 'tmhomebannercms';
		$this->tab = 'Other';
		$this->version = '1.0';
		...
		$this->displayName = $this->l('Templatemela - Banner CMS Block');
	...
	function hookdisplayHome($params){return $this->hookdisplayTemplateMelaCMSContent($params);}

	public function hookdisplayTemplateMelaCMSContent($params)
	{
		$this->context->controller->addCSS($this->_path.'tmcmscontent.css', 'all');
		...
		if (file_exists(dirname(__FILE__).'/content.xml'))
		{
			...
			if ($xml = simplexml_load_file(dirname(__FILE__).'/content.xml'))
			{
				...
				return $this->display(__FILE__, 'tmhomebannercms.tpl');
			}
		}
		return false;
	}

...
include_once(_PS_MODULE_DIR_.'tmsubbanner/subbanner.php');

class TmSubBanner extends Module
{
	private $_html = '';

	public function __construct()
	{
		$this->name = 'tmsubbanner';
		$this->tab = 'Other';
		$this->version = '1.0';
		...
		$this->displayName = $this->l('Templatemela - HomePage Subbanner');
	...
	private function installSamples()
	{
		...
		for ($i = 1; $i <= 2; ++$i)
		{
			...
			{
				$slide->title[$language['id_lang']] = 'SubBanner '.$i;
				$slide->url[$language['id_lang']] = 'http://www.templatemela.com/';
				$slide->image[$language['id_lang']] = 'sub-banner-'.$i.'.jpg';
			}
			$slide->add();
		}
	}
	...
	protected function createTables()
	{
		...
		$res = (bool)Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'tmsubbanner` (
				`id_tmsubbanner_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
				`id_shop` int(10) unsigned NOT NULL,
				PRIMARY KEY (`id_tmsubbanner_slides`, `id_shop`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');

		/* Slides configuration */
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'tmsubbanner_slides` (
			  `id_tmsubbanner_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
			  `position` int(10) unsigned NOT NULL DEFAULT \'0\',
			  `active` tinyint(1) unsigned NOT NULL DEFAULT \'0\',
			  PRIMARY KEY (`id_tmsubbanner_slides`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');

		/* Slides lang configuration */
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'tmsubbanner_slides_lang` (
			  `id_tmsubbanner_slides` int(10) unsigned NOT NULL,
			  `id_lang` int(10) unsigned NOT NULL,
			  `title` varchar(255) NOT NULL,
			  `url` varchar(255) NOT NULL,
			  `image` varchar(255) NOT NULL,
			  PRIMARY KEY (`id_tmsubbanner_slides`,`id_lang`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');
	...
	protected function deleteTables()
	{
		...
		{
			$to_del = new SubBanner($slide['id_slide']);
			$to_del->delete();
		}
		return Db::getInstance()->execute('
			DROP TABLE IF EXISTS `'._DB_PREFIX_.'tmsubbanner`, `'._DB_PREFIX_.'tmsubbanner_slides`, `'._DB_PREFIX_.'tmsubbanner_slides_lang`;
		');
	}
	...
	private function _displayAddForm()
	{
		...
		if (Tools::isSubmit('id_slide') && $this->slideExists((int)Tools::getValue('id_slide')))
			$slide = new SubBanner((int)Tools::getValue('id_slide'));
		...
		foreach ($languages as $language)
		{
			...
			/* Display image */
			if ($slide && $slide->image[$language['id_lang']])
				$this->_html .= '<input type="hidden" name="has_picture" value="1" /><img src="'.__PS_BASE_URI__.'modules/'.$this->name.'/images/'.$slide->image[$language['id_lang']].'" width="350px" height="150px" alt=""/>';
			$this->_html .= '</div>';
		}
	...
	private function _postProcess()
	{
		...
		elseif (Tools::isSubmit('submitSlide'))
		{
			...
			if (!$errors)
			{
				...
				{
					if (!$slide->add())
						$errors[] = $this->displayError($this->l('Banner could not be added'));
				} /* Update */
				elseif (!$slide->update())
					$errors[] = $this->displayError($this->l('Banner could not be updated'));
			}
		} /* Deletes */
		elseif (Tools::isSubmit('delete_id_slide'))
		{
			$slide = new SubBanner((int)Tools::getValue('delete_id_slide'));
			$res = $slide->delete();
			if (!$res)
	...
	private function _prepareHook()
	{
		...
		$this->smarty->assign('tmsubbanner_slides', $slides);
	...
	public function hookDisplayHome()
	{
		...
		return $this->display(__FILE__, 'tmsubbanner.tpl');
	}

	public function hookActionShopDataDuplication($params)
	{
		Db::getInstance()->execute('
		INSERT IGNORE INTO '._DB_PREFIX_.'tmsubbanner (id_tmsubbanner_slides, id_shop)
		SELECT id_tmsubbanner_slides, '.(int)$params['new_id_shop'].'
		FROM '._DB_PREFIX_.'tmsubbanner
	...
	public function getNextPosition()
	{
		$row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
				...
				FROM `'._DB_PREFIX_.'tmsubbanner_slides` hss, `'._DB_PREFIX_.'tmsubbanner` hs
				WHERE hss.`id_tmsubbanner_slides` = hs.`id_tmsubbanner_slides` AND hs.`id_shop` = '.(int)$this->context->shop->id
		);
	...
	public function getSlides($active = null)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hs.`id_tmsubbanner_slides` as id_slide,
					   hssl.`image`,
					   hss.`position`,
					   hss.`active`,
					   hssl.`title`,
					   hssl.`url`
			FROM '._DB_PREFIX_.'tmsubbanner hs
			LEFT JOIN '._DB_PREFIX_.'tmsubbanner_slides hss ON (hs.id_tmsubbanner_slides = hss.id_tmsubbanner_slides)
			LEFT JOIN '._DB_PREFIX_.'tmsubbanner_slides_lang hssl ON (hss.id_tmsubbanner_slides = hssl.id_tmsubbanner_slides)
	...
	{
		$req = 'SELECT hs.`id_tmsubbanner_slides` as id_slide
				FROM `'._DB_PREFIX_.'tmsubbanner` hs
				WHERE hs.`id_tmsubbanner_slides` = '.(int)$id_slide;

...
class tmpolicycms extends Module
{
	function __construct()
	{
		$this->name = 'tmpolicycms';
		$this->tab = 'Other';
		$this->version = '1.0';
		...
		$this->displayName = $this->l('Templatemela - Footer Policy Block');
	...
	public function hookdisplayRightColumn($params)
	{
		...
		if (file_exists(dirname(__FILE__).'/content.xml'))
		{
			...
			if ($xml = simplexml_load_file(dirname(__FILE__).'/content.xml'))
			{
				...
				return $this->display(__FILE__, 'tmpolicycms.tpl');
			}
		}
		return false;
	}

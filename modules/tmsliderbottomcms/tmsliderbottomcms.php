...
class tmsliderbottomcms extends Module
{
	function __construct()
	{
		$this->name = 'tmsliderbottomcms';
		$this->tab = 'Other';
		$this->version = '1.0';
		...
		$this->displayName = $this->l('Templatemela - Slider Bottom Cms');
		$this->description = $this->l('Add CMS Content anywhere');
	}
	
	public function install()
	{
		if (!parent::install()	|| !$this->registerHook('TemplateMelaSlider') )
			return false;
		return true;
	}
	...
	function hookTemplateMelaSlider($params){return $this->hookdisplayRightColumn($params);}
	...
	public function hookdisplayRightColumn($params)
	{
		...
		if (file_exists(dirname(__FILE__).'/content.xml'))
		{
			...
			if ($xml = simplexml_load_file(dirname(__FILE__).'/content.xml'))
			{
				...
				return $this->display(__FILE__, 'tmsliderbottomcms.tpl');
			}
		}
		return false;
	}
